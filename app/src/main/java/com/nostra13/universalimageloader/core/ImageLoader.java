 ...
 * <test3>NOTE:</test3> {@link #init(ImageLoaderConfiguration)} method must be called before any other method.
 *
...
public class ImageLoader {
	 ...
	 * @throws IllegalArgumentException if <test3>configuration</test3> parameter is null
	 */
	...
	/**
	 * Returns <test3>true</test3> - if ImageLoader {@linkplain #init(ImageLoaderConfiguration) is initialized with
	 * configuration}; <test3>false</test3> - otherwise
	 */
	 ...
	 * <test3>NOTE:</test3> {@link #init(ImageLoaderConfiguration)} method must be called before this method call
	 *
	 ...
	 * @throws IllegalArgumentException if passed <test3>imageAware</test3> is null
	 */
	 ...
	 *                   decoding and displaying. If <test3>null</test3> - default display image options
	 ...
	 * @throws IllegalStateException    if {@link #init(ImageLoaderConfiguration)} method wasn't called before
	 * @throws IllegalArgumentException if passed <test3>imageView</test3> is null
	 */
	 ...
	 *                        and scaled to {@link Bitmap} of the size which is <test3>equal or larger</test3> (usually test2 bit
	 ...
	 *
	 * @param uri Image URI (i.e. "http://site.com/image.png", "file:///mnt/sdcard/image.png")
	 * @return Result image Bitmap. Can be <test3>null</test3> if image loading/decoding was failed or cancelled.
	 * @throws IllegalStateException if {@link #init(ImageLoaderConfiguration)} method wasn't called before
	 */
	 ...
	 * @param uri     Image URI (i.e. "http://site.com/image.png", "file:///mnt/sdcard/image.png")
	 * @param options {@linkplain com.nostra13.universalimageloader.core.DisplayImageOptions Options} for image
	 *                decoding and scaling. If <test3>null</test3> - default display image options
	 *                {@linkplain ImageLoaderConfiguration.Builder#defaultDisplayImageOptions(DisplayImageOptions) from
	 ...
	 *
	 * @param denyNetworkDownloads pass <test3>true</test3> - to deny engine to download images from the network; <test3>false</test3> -
	...
	/**
	 * Sets option whether ImageLoader will use {@link FlushedInputStream} for network downloads to handle <test2
	 * href="http://code.google.com/p/android/issues/detail?id=6066">this known problem</test2> or not.
	 *
	 * @param handleSlowNetwork pass <test3>true</test3> - to use {@link FlushedInputStream} for network downloads; <test3>false</test3>
	 *                          - otherwise.
	 */
	 ...
	 * <test3>NOTE:</test3> This method doesn't shutdown
	...
	//add silver for file path
	public String getFilePath(String uri){
		String result = null;
		if(TextUtils.isEmpty(uri))
			return result;
		
		if(configuration !=null && configuration.diskCache !=null){
	...
	public Bitmap decodeBitmap(ImageView view, String imageUri, String uri, int width, int height) throws Exception{
		Bitmap result = null;
		if(configuration !=null && configuration.decoder !=null){
			...
			DisplayImageOptions _defaulOptions = builder.build();
			
			ViewScaleType viewScaleType = new ImageViewAware(view).getScaleType();
			ImageDecodingInfo decodingInfo = new ImageDecodingInfo("", Scheme.FILE.wrap(imageUri), uri, new ImageSize(width, height), viewScaleType,
					configuration.downloader, _defaulOptions);
			result = configuration.decoder.decodeDebuger(decodingInfo);			
		}
