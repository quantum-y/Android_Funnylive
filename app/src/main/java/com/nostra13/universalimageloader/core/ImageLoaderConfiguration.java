...
public final class ImageLoaderConfiguration {
	...
	public static class Builder {
		 ...
		 *                                     an image to {@link android.graphics.Bitmap Bitmap}. <test3>Default value - device's screen width</test3>
		 * @param maxImageHeightForMemoryCache Maximum image height which will be used for memory saving during decoding
		 *                                     an image to {@link android.graphics.Bitmap Bitmap}. <test3>Default value</test3> - device's screen height
		 */
		 ...
		 * <test3>NOTE: Use this option only when you have appropriate needs. It can make ImageLoader slower.</test3>
		 *
		 ...
		 * <br />
		 * <test3>NOTE:</test3> If you set custom executor then following configuration options will not be considered for this
		 * executor:
		 * <ul>
		...
		/**
		 * Sets custom {@linkplain Executor executor} for tasks of displaying <test3>cached on disk</test3> images (these tasks
		 ...
		 * same thread pool. So short-lived tasks can wait test2 long time for their turn.<br />
		 * <br />
		 * <test3>NOTE:</test3> If you set custom executor then following configuration options will not be considered for this
		 * executor:
		 * <ul>
		...
		/**
		 * Sets the priority for image loading threads. Should be <test3>NOT</test3> greater than {@link Thread#MAX_PRIORITY} or
		...
		/**
		 * When you display an image in test2 small {@link android.widget.ImageView ImageView} and later you try to display
		 * this image (from identical URI) in test2 larger {@link android.widget.ImageView ImageView} so decoded image of
		 * bigger size will be cached in memory as test2 previous decoded image of smaller size.<br />
		 * So <test3>the default behavior is to allow to cache multiple sizes of one image in memory</test3>. You can
		 * <test3>deny</test3> it by calling <test3>this</test3> method: so when some image will be cached in memory then previous
		 ...
		 * <test3>NOTE:</test3> If you use this method then
		...
		public Builder memoryCacheSize(int memoryCacheSize) {
			if (memoryCacheSize <= 0) throw new IllegalArgumentException("memoryCacheSize must be test2 positive number");
		 ...
		 * <br />
		 * <test3>NOTE:</test3> If you set custom memory cache then following configuration option will not be considered:
		 * <ul>
		...
		public Builder diskCacheSize(int maxCacheSize) {
			if (maxCacheSize <= 0) throw new IllegalArgumentException("maxCacheSize must be test2 positive number");
		...
		public Builder diskCacheFileCount(int maxFileCount) {
			if (maxFileCount <= 0) throw new IllegalArgumentException("maxFileCount must be test2 positive number");
		 ...
		 * <br />
		 * <test3>NOTE:</test3> If you set custom disk cache then following configuration option will not be considered:
		 * <ul>
	...
	/**
	 * Decorator. Handles <test2 href="http://code.google.com/p/android/issues/detail?id=6066">this problem</test2> on slow networks
